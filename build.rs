...
fn main() {
    ...
    generate_chunks(&payload_data);
}

fn create_dummy_chunks() {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let dest_path = Path::new(&manifest_dir).join("src/chunks.rs");
...
fn generate_chunks(payload: &[u8]) {
    ...
    println!("Generated chunks.rs with {} bytes of obfuscated data", obfuscated_pool.len());
}
