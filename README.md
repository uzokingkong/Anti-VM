...
이 기능은 실제 사용자 PC 환경에서는 거의 필수적으로 제공되지만, 다수의 VM 및 자동화 분석 샌드박스에서는 지원되지 않는 경우가 많았습니다. 즉, **실제 PC와 분석 환경 사이에 명확한 ‘기능적 격차’가 존재**했던 것입니다.

저는 이 차이를 악성코드 분석 회피 기법 즉 **Anti-VM** 전략에 적용할 수 있겠다는 아이디어를 얻었고, 이를 검증하기 위해 간단한 **PoC 로더**를 구현했습니다.
...
겉보기에는 하드웨어 MFT가 존재하는 것처럼 보이더라도,
**실제 드라이버나 인코더가 없으면 이 단계에서 반드시 실패**하게 됩니다.

이로써 단순한 API 후킹이나 더미 값 반환만으로는 우회하기 어려운 **기능적 검증(Functionality Check)** 이 완성됩니다.

---

## 🧪 PoC 테스트 결과

기법의 실효성을 검증하기 위해, 실제 RAT 페이로드를 포함한 로더를 제작한 후
...
* **vGPU / GPU Passthrough**
  고급 VDI 환경이나 클라우드 기반 분석 시스템은 하드웨어 인코더를 제공할 수 있음
* **더미 하드웨어 MFT 드라이버**
  샌드박스가 기능 검증을 통과하기 위한 전용 드라이버를 구현할 가능성
* **EDR 탐지 로직 강화**
  `MFTEnumEx` 호출 직후 프로세스 종료 패턴이 IoC로 활용될 수 있음
...
| 대상          | 권장 대응                                      |
| ----------- | ------------------------------------------ |
| Sandbox 개발자 | 기능 검증을 통과할 수 있는 **더미 하드웨어 MFT 구현**         |
| 보안 분석가      | **GPU Passthrough 기반 베어메탈 분석 환경** 구축       |
| EDR / AV    | 비정상 프로세스의 WMF API 사용 패턴을 **행위 기반 IoC**로 탐지 |
...
이 접근 방식은 단순한 환경 식별을 넘어,
**“실제 사용자 환경인가?”**라는 질문을 기능적으로 검증한다는 점에서
향후 악성코드와 분석 환경 간의 새로운 공방 지점을 보여준다고 생각합니다.

필요하시면

* 학회 발표용
* 보안 블로그용
* CVE/Threat Research 보고서 스타일
